# Contributing to AmbientVRT

Thanks for your interest in contributing! This guide covers the repository
layout, local development, and the CI workflows. For what AmbientVRT *is* and
how to use it, see the [README](README.md).

AmbientVRT is framework-agnostic visual regression testing: capture
component/preview snapshots, compare them against blessed baselines, and gate
changes in CI. One core engine, pluggable capture adapters (Flutter and React
Native).

Contributions are welcome via pull request. Please run `melos run analyze` and
the test suites (below) before opening one, and keep changes scoped to a single
concern.
